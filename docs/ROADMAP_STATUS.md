...
| 5 Economy & Shops | IMPLEMENTIERT / RUNTIME-TRANSACTION-GATE | Systemshops, PvP-Shop, PlayerShop, Trade, Merchant/Black Market und geschuetzte Market-Rentals vorhanden. PlayerShop reserviert Stock vor Ausgabe, Pending Revenue vor Claim und blockiert `long`-Overflows bei Gebuehr, Revenue-Akkumulation und Auszahlung fail-closed. Haendler-Eier sind ohne per-Item-UUID stackbar; Legacy-Eier bleiben erkennbar. Trade-Sessions sind atomar und jede Angebotsaenderung invalidiert beide Zusagen. Runtime-Race-/Balance-Test bleibt Gate. |
...
Die automatisierbaren Punkte des zuletzt abgearbeiteten Hardening-Fahrplans sind code-seitig umgesetzt. Die verbleibenden Punkte sind absichtlich echte Runtime-/Map-/Multiplayer-Gates und keine weiteren blinden Feature-Baustellen:
...
8. PlayerShop mit zwei Spielern testen: letzter Stock, voller Inventory-Fall, Revenue-Doppelclaim, Restart-Fenster sowie die neuen Overflow-Guards
...
18. Trade-Offer waehrend Countdown aendern, Quit/Close und Coin-Aenderungen testen
...
25. danach Soft-Launch-Gate statt weitere Kernsysteme blind aufzubauen
