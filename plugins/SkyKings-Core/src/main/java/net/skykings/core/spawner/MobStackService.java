...
/**
 * Leichtgewichtiges Mob-Stacking fuer Spawner-Mobs auf SkyIslands/SkyPlots.
 *
 * Statt dutzende identische Entities gleichzeitig zu halten, werden nahe Spawner-Mobs
 * desselben Typs zu einem sichtbaren Stack zusammengefasst. Bei jedem Kill wird genau
 * ein Mob normal ausgewertet (Drops/XP bleiben vanilla) und der Reststack wird im
 * naechsten Tick wiederhergestellt. Der Stackzaehler steckt im Entity-Namen und bleibt
 * dadurch auch ueber Chunk-Unload/Restart erhalten.
 */
public final class MobStackService implements Listener {
    ...
    public void onSpawn(CreatureSpawnEvent event) {
        ...
        // Einen Tick warten, damit das Entity vollstaendig in der Welt registriert ist.
        plugin.getServer().getScheduler().runTask(plugin, new Runnable() {
            @Override public void run() {
                if (spawned == null || spawned.isDead() || !spawned.isValid()) return;
                mergeNearby(spawned);
            }
        });
    }
    ...
    public void onDeath(EntityDeathEvent event) {
        ...
        // Der aktuelle Kill behaelt seine normalen Drops/XP. Nur der Reststack kommt zurueck.
        plugin.getServer().getScheduler().runTask(plugin, new Runnable() {
            @Override public void run() {
                ...
                LivingEntity living = (LivingEntity) replacement;
                applyCount(living, remaining);
    ...
    private void mergeNearby(LivingEntity target) {
        ...
        for (Entity entity : target.getNearbyEntities(MERGE_RADIUS, MERGE_RADIUS, MERGE_RADIUS)) {
            ...
            if (other.getType() != target.getType()) continue;
            if (!eligibleName(other)) continue;
