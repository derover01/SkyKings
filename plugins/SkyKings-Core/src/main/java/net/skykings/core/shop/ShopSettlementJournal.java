...
public final class ShopSettlementJournal {
    ...
    public synchronized UUID begin(UUID player, String shopId, String offerId, String currency,
                                   long price, String rewardMaterial, int rewardAmount) {
        if (player == null || offerId == null || currency == null || price <= 0L
                || rewardMaterial == null || rewardAmount <= 0) return null;
        UUID transaction = UUID.randomUUID();
        String base = "settlements." + transaction + ".";
        data.set(base + "status", "IN_PROGRESS");
        data.set(base + "player", player.toString());
        data.set(base + "shop", shopId == null ? "unknown" : shopId);
        data.set(base + "offer", offerId);
        data.set(base + "currency", currency);
        data.set(base + "price", price);
        data.set(base + "reward-material", rewardMaterial);
        data.set(base + "reward-amount", rewardAmount);
    ...
    private void cleanupCompleted() {
        ...
        if (!commit()) logger.warning("Shop-Settlement-Journal konnte COMPLETED-Tombstones nicht aufraeumen.");
    }
    ...
    private boolean commit() {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Shop-Settlement-Journal konnte nicht gespeichert werden.", ex);
