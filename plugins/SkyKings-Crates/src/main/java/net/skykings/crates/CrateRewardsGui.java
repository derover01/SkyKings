...
/** /craterewards GUI mit eigenem Cooldown pro Paid-Rank-Tier. */
public final class CrateRewardsGui {
    ...
    private static final Tier[] TIERS = new Tier[] {
            ...
            new Tier(Rank.PHOENIX, 4L, "common", 2, Material.BLAZE_POWDER),
            new Tier(Rank.ETERNAL, 8L, "rare", 1, Material.ENDER_PEARL),
            new Tier(Rank.EXILE, 12L, "rare", 2, Material.OBSIDIAN),
            new Tier(Rank.ENDLING, 18L, "rare", 3, Material.ENDER_STONE),
            new Tier(Rank.KING, 24L, "rare", 4, Material.GOLD_BLOCK)
    };
    ...
    public CrateRewardsGui(GuiManager guiManager, SkyKingsCoreAPI core,
                           CrateRegistry registry, CrateItemCodec codec) {
    ...
    private void claim(Player player, Tier tier) {
        if (!player.isOp() && !core.getRankService().hasAtLeast(player.getUniqueId(), tier.rank)) {
            player.sendMessage(ChatColor.RED + "Du benoetigst mindestens " + display(tier.rank) + ".");
            return;
        }
        String key = key(tier);
        ...
        if (crate == null) {
            player.sendMessage(ChatColor.RED + "Der konfigurierte Crate-Typ ist aktuell nicht verfuegbar.");
            return;
        }
    ...
    private ItemStack icon(Player player, Tier tier) {
        ...
        boolean access = player.isOp() || core.getRankService().hasAtLeast(player.getUniqueId(), tier.rank);
    ...
    private ItemStack info() {
        ...
        ItemMeta meta = item.getItemMeta();
        meta.setDisplayName(ChatColor.AQUA + "So funktionieren Crate Rewards");
        meta.setLore(Arrays.asList(
                ChatColor.GRAY + "Jeder Paid-Rang hat einen eigenen Reward.",
                ChatColor.GRAY + "Hoehere Raenge koennen auch niedrigere",
                ChatColor.GRAY + "Rank-Rewards mit deren eigenem Cooldown claimen."));
        item.setItemMeta(meta);
        return item;
    }

    private String key(Tier tier) {
        return "craterewards:" + tier.rank.name().toLowerCase(Locale.ROOT);
    }
